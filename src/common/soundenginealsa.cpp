...
static void low_pass_filter(short *data, int numframes, float f)
{
  static short prevs[2]={0,0};
  for (int i=0; i<numframes; i++)
  {
    short cur0 = data[2*i+0];
    data[2*i+0] = (short) ((1-f)*cur0 + f*prevs[0]);
    short cur1 = data[2*i+1];
    data[2*i+1] = (short) ((1-f)*cur1 + f*prevs[1]);
    prevs[0]=data[2*i+0];
    prevs[1]=data[2*i+1];
  }
}
...
SoundEngineAlsa::SoundEngineAlsa(int lag) :
  opened(false),
  simplefeed(0),
  complexfeed(0),
  enginefeed(0),
